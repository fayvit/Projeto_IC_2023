...
{
    public struct LimitantesDeMapa
    {
        public int xMin;
        public int xMax;
        public int yMin;
    ...
    private static Color CorAlvoMap(int row, int col, LimitantesDeMapa L,Dictionary<MyVector2Int, MyColor> GetMapDates)
    {
    ...
    private static LimitantesDeMapa CalculaLimitanteMapBase(Dictionary<MyVector2Int,MyColor> GetMapDates)
    {
    ...
    private static LimitantesDeMapa CalculaLimitanteMap(Dictionary<MyVector2Int, MyColor> GetMapDates)
    {
        LimitantesDeMapa L = CalculaLimitanteMapBase(GetMapDates);
    ...
    {
        return TexturaDeMapaAtual(GetMapDates);
    }

    public static Texture2D TexturaDeMapaAtual(Dictionary<MyVector2Int, MyColor> GetMapDates)
    {
        LimitantesDeMapa L = CalculaLimitanteMap(GetMapDates);

        Texture2D tex = new Texture2D(L.xMax - L.xMin, L.yMax - L.yMin);
        for (int row = 0; row < L.yMax - L.yMin; row++)
            for (int col = 0; col < L.xMax - L.xMin; col++)
                tex.SetPixel(col, row, CorAlvoMap(row, col, L,GetMapDates));
